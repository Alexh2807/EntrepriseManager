...
extends SubCommand {
    ...
    public boolean perform(Player player, String[] args) throws ExecutionException, InterruptedException {
        ...
        if (args.length == 2) {
            ...
            if (!player.hasPermission(PermissionPath.BankShow.getPerm()) && !player.isOp()) {
            ...
            for (String s : messages.getStringList("bankShowOther")) {
                ...
                player.sendMessage(Main.colorTranslation.hexTranslation(s.replace("#target#", args[1]).replace("#bank-balance#", Main.util.finalFormatDouble(currentBankBalance)).replace("#currency#", Main.util.getCurrency(currentBankBalance)).replace("#limit-by-level#", Main.util.finalFormatDouble(currentLimit)).replace("#current-bank-level#", String.valueOf(currentLevel)).replace("#max-level#", String.valueOf(maxLevelViaConfig))));
            }
            return false;
        }
