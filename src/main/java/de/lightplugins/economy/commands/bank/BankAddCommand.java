    ...
    public boolean perform(Player player, String[] args) throws ExecutionException, InterruptedException {
        if (args.length == 3) {
            ...
            try {
                ...
                if (!player.hasPermission(PermissionPath.BankAdd.getPerm()) && !player.isOp()) {
                ...
                if (limit < currentBankBalance + addValue) {
                    Main.util.sendMessage(player, MessagePath.BankAddPlayerLimit.getPath().replace("#limit#", Main.util.finalFormatDouble(limit)));
                    sounds.soundOnFailure(player);
                    return false;
                }
                ...
                try {
                    if (completableFuture.get().booleanValue()) {
                        Main.util.sendMessage(player, MessagePath.BankAddPlayer.getPath().replace("#amount#", Main.util.finalFormatDouble(addValue)).replace("#currency#", Main.util.getCurrency(addValue)).replace("#target#", targetName));
