    ...
    public boolean perform(Player player, String[] args) throws ExecutionException, InterruptedException {
        if (args.length == 3) {
            ...
            try {
                ...
                if (!player.hasPermission(PermissionPath.BankSet.getPerm()) && !player.isOp()) {
                ...
                if (limit < setValue) {
                    Main.util.sendMessage(player, MessagePath.BankSetPlayerLimit.getPath().replace("#limit#", Main.util.finalFormatDouble(limit)));
                    sounds.soundOnFailure(player);
                    return false;
                }
                ...
                try {
                    if (completableFuture.get().booleanValue()) {
                        Main.util.sendMessage(player, MessagePath.BankSetPlayer.getPath().replace("#amount#", Main.util.finalFormatDouble(setValue)).replace("#currency#", Main.util.getCurrency(setValue)).replace("#target#", targetName));
