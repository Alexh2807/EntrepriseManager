...
public class MoneyAddAllCommand
extends SubCommand {
    @Override
    ...
    public boolean perform(Player player, String[] args) throws ExecutionException, InterruptedException {
        ...
        if (!player.hasPermission(PermissionPath.MoneyAddAll.getPerm()) && !player.isOp()) {
        ...
        for (Map.Entry<String, Double> entry : map.entrySet()) {
            ...
            if (!respo.transactionSuccess()) {
                Main.util.sendMessage(player, MessagePath.TransactionFailed.getPath().replace("#reason#", respo.errorMessage + " - " + entry.getKey()));
            }
            ++successCounter;
            Player target = Bukkit.getPlayer((String)entry.getKey());
            assert (target != null);
            Main.util.sendMessage(target, MessagePath.moneyAddAllTarget.getPath().replace("#sender#", player.getName()).replace("#amount#", args[1]).replace("#currency#", Main.util.getCurrency(amount)));
        }
        Main.util.sendMessage(player, MessagePath.moneyAddAll.getPath().replace("#successcount#", String.valueOf(successCounter)).replace("#overallcount#", String.valueOf(map.size())).replace("#amount#", Main.util.finalFormatDouble(amount)).replace("#currency#", Main.util.getCurrency(amount)));
        if (successCounter < map.size()) {
            Main.util.sendMessage(player, MessagePath.moneyAddAllFailed.getPath().replace("#failcount#", String.valueOf(map.size() - successCounter)));
        }
        return false;
    }
}
