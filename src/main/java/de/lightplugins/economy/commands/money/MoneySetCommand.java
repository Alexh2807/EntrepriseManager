...
extends SubCommand {
    ...
    public boolean perform(Player player, String[] args) {
        ...
        if (!player.hasPermission(PermissionPath.MoneySet.getPerm()) && !player.isOp()) {
        ...
        try {
            ...
            moneyTableAsync.playerBalance(args[1]).thenAccept(result -> moneyTableAsync.setMoney(args[1], amount).thenAccept(success -> Main.util.sendMessage(player, MessagePath.MoneySetPlayer.getPath().replace("#currency#", Main.util.getCurrency(amount)).replace("#target#", args[1]).replace("#amount#", Main.util.finalFormatDouble(amount)))));
