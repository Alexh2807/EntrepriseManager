...
public class DatabaseConnection {
    ...
    public void connectToDatabaseViaSQLite() {
        ...
        File dataFolder = new File(this.plugin.getLightEconomyDataFolder(), database);
        if (!dataFolder.exists()) {
            try {
                if (!dataFolder.createNewFile()) {
                    this.plugin.getLogger().log(Level.SEVERE, "Cannot create " + database);
                    return;
                }
            } catch (IOException e) {
                this.plugin.getLogger().log(Level.SEVERE, "Cannot create " + database);
        ...
        this.plugin.ds = new HikariDataSource(config);
